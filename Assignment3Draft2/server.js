...
// Borrowed and modified from Lab 14, Prof. Port's screencasts 
    if (fs.existsSync(user_info_file)) { // Check if the user info file exists
        stats = fs.statSync(user_info_file) // Load in the user_data file to user_data object!
        console.log(user_info_file + ' has ' + stats.size + ' characters!'); // Outputs # of characters
        data = fs.readFileSync(user_info_file, 'utf-8');
        var user_data= JSON.parse(data); // Returns string, parses into object
    } else {
...
// Borrowed and modified from Lab 14 exercise and some from Alyssa Mencel (Fall 2020)
    app.post("/process_login", function (req, res, next) {
        ...
        username = req.body.username.toLowerCase(); // Username should be all lowercase
        if (typeof user_data[username] != 'undefined') { // Username and password should not be undefined
            if (user_data[username].password == req.body.password) {
                ...
                res.cookie('user_info', JSON.stringify(user_info));
...
// Borrowed and modified from Lab 14 //
    app.post("/process_register", function (req, res) {
        ...
        username = req.body.username.toLowerCase(); // Username should be all lowercase
        if (typeof user_data[username] != 'undefined') { // Gives error if username is taken and displays message 'Username taken'
...
// Borrowed the res.clearCookie line from Kimberly Matutina 
    app.get("/logout", function (req, res) {
        var username = user_data[username].name;
        user_data[username].name = username;
        var user_info = {"username": username, "name": user_data[username].name, "email": user_data[username].email};
        // Nicholas Samson helped me personalize the user's username
        str = `<script>alert('${user_info["name"]} has successfully logged out!'); location.href="./index.html";</script>`; 
